...
public sealed class StockMarketSystem : EntitySystem
{
        ...
        EntityUid user)
    {
        ...
        _adminLogger.Add(LogType.Action,
            LogImpact.Medium,
            $"[StockMarket] {ToPrettyString(user):user} {verb} {Math.Abs(amount)} stocks of {company.LocalizedDisplayName} at {company.CurrentPrice:F2} credits each (Total: {totalValue})"); // Bugstation - changed to credits
